//===========================================================================
// apache2_tls_access_file_parse.c
...
//===========================================================================
// apache2_tls_access_do
//===========================================================================
PUBLIC	s32 apache2_tls_access_do(s8 format)
{
	FILE *fp_r;
	s32 num;
	s32 len;
	...
	strcpy_2(sql_exe, "create table if not exists tmp_apache2_tls_access_log(remote_ip char(32), time DATETIME, url char(128), protocol char(32), res_state int, send_bytes int)");
	...
	// 分析日志文件
	//-----------------------------------------------------------------
	fp_r 	= fopen(apa_tls_access_log_path,"r");
	...
	while(fgets(buf_line, num ,fp_r) != NULL)
	{
		...
		//3)构建sql语句
		strcpy_2(sql_exe, "insert into tmp_apache2_tls_access_log values (");
