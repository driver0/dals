...
PUBLIC	s32 sshd_do(s8 format)
{
	...
	strcpy_2(sql_exe, "create table if not exists tmp_sshd_log(time DATETIME, content varchar(512))");
	...
	while(fgets(buf_line, num ,fp_r) != NULL)
	{
		...
		struct sshd_log_1 *dl	= buf_line;	

		//3)构建sql语句
		strcpy_2(sql_exe, "insert into tmp_sshd_log values (");
