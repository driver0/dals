...
PUBLIC	s32 apache2_error_do(s8 format)
{
	...
	strcpy_2(sql_exe, "create table if not exists tmp_apache2_error_log(time DATETIME, content char(128))");
	...
	// 分析日志文件
	//-----------------------------------------------------------------
	fp_r 	= fopen(apa_error_log_path,"r");
	...
	while(fgets(buf_line, num ,fp_r) != NULL)
	{
		...
		//3)构建sql语句
		strcpy_2(sql_exe, "insert into tmp_apache2_error_log values (");
