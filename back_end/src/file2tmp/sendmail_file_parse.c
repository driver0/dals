...
PUBLIC	s32 sendmail_do(s8 format)
{
	...
	strcpy_2(sql_exe, "create table if not exists tmp_sendmail_log(time DATETIME, content varchar(512))");
	...
	// 分析日志文件
	//-----------------------------------------------------------------
	fp_r 	= fopen(sendmail_log_path,"r");
	...
	while(fgets(buf_line, num ,fp_r) != NULL)
	{
		...
		// 2017-06-26T10:39:18.793492+08:00 test postfix/pickup[7615]: C1A1B10475E: uid=0 from=<kkk>
		struct sendmail_log_1 *sl	= buf_line;	

		//3)构建sql语句
		strcpy_2(sql_exe, "insert into tmp_sendmail_log values (");

		//add datetime
		//将日志里的日期格式转换为mysql里的DATETIIME格式
		// 2017-06-26T10:28:29.672151+08:00	==>	2017-06-26 10:28:29
		todatetime_6(tmp_buf, 32, &(sl->dt));		
