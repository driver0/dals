...
PUBLIC	s32 vsftpd_exec()
{
	...
	strcpy_2(sql_exe, "create table if not exists vsftpd_log(time DATETIME, content char(128))");
	...
	strcpy_2(sql_exe, "select * from vsftpd_log");
	res = db_exe(conn, sql_exe);

	s32 num_cnt = mysql_fetch_row(res);

	//-----------------------------------------------------------------
	// 将tmp_vsftpd_log 里的数据放入 vsftpd_log
	//-----------------------------------------------------------------
	memset(sql_exe, NULL, 1024);

	if(num_cnt == 0)
	{
		strcpy_2(sql_exe, "insert into vsftpd_log select * from tmp_vsftpd_log");
	}
	else
	{
		memset(sql_exe, NULL, 1024);

		strcpy_2(sql_exe, "select max(time) from vsftpd_log");
		res = db_exe(conn, sql_exe);

		MYSQL_ROW sqlrow = mysql_fetch_row(res);

		strcpy_2(sql_exe, "insert into vsftpd_log select * from tmp_vsftpd_log where time > '");
		strcat(sql_exe, sqlrow[0]);
		strcat(sql_exe, "'");

		printf("vsftpd_log::sql_exe=%s\n",sql_exe);
	}
	...
	strcpy_2(sql_exe, "drop table tmp_vsftpd_log");
