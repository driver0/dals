...
PUBLIC	s32 dns_exec()
{
	...
	strcpy_2(sql_exe, "create table if not exists dns_log(time DATETIME, content varchar(512))");
	...
	strcpy_2(sql_exe, "select * from dns_log");
	res = db_exe(conn, sql_exe);

	s32 num_cnt = mysql_fetch_row(res);

	//-----------------------------------------------------------------
	// 将tmp_dns_log 里的数据放入 dns_log
	//-----------------------------------------------------------------
	memset(sql_exe, NULL, 1024);

	if(num_cnt == 0)
	{
		strcpy_2(sql_exe, "insert into dns_log select * from tmp_dns_log");
	}
	else
	{
		memset(sql_exe, NULL, 1024);

		strcpy_2(sql_exe, "select max(time) from dns_log");
		res = db_exe(conn, sql_exe);

		MYSQL_ROW sqlrow = mysql_fetch_row(res);

		strcpy_2(sql_exe, "insert into dns_log select * from tmp_dns_log where time > '");
		strcat(sql_exe, sqlrow[0]);
		strcat(sql_exe, "'");

		printf("dns_log::sql_exe=%s\n",sql_exe);
	}
	...
	strcpy_2(sql_exe, "drop table tmp_dns_log");
